  ...
  <Dialog :open="open" @update:open="handleOpenChange">
    <DialogContent class="sm:max-w-[600px]">
      ...
      <div class="py-4" v-if="networkDataClone">
        <div class="space-y-3">
          <div v-for="(pool, index) in networkDataClone.ipAssignmentPools" :key="index" class="flex items-center gap-2">
            <Input type="text" v-model="pool.ipRangeStart" :placeholder="t('network.ipAssignmentPools.ipRangeStart')"
              class="flex-1 border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
              autocomplete="off" />
            <Input type="text" v-model="pool.ipRangeEnd" :placeholder="t('network.ipAssignmentPools.ipRangeEnd')"
              class="flex-1 border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
              autocomplete="off" />
