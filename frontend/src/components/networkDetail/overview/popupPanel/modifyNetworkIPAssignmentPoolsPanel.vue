...
const networkData = ref(
    _.cloneDeep(networkDetailStore.networksData.find((data) => data.id === (networkId as string))),
)
...
<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 popup-panel"
        ...
        <div class="bg-white rounded p-5 flex flex-col justify-between lg:w-1/3">
            ...
            <div class="flex mb-2" v-for="(ipAssignment, index) in networkData?.ipAssignmentPools" :key="index">
                <input type="text" :placeholder="t('network.ipAssignmentPools.ipRangeStart')"
                    class="focus:outline-none border-b w-full mr-1" name="ipRangeStart"
                    v-model="ipAssignment.ipRangeStart" autocomplete="off" />
                <input type="text" :placeholder="t('network.ipAssignmentPools.ipRangeEnd')"
                    class="focus:outline-none border-b w-full" name="ipRangeEnd" v-model="ipAssignment.ipRangeEnd"
                    autocomplete="off" />
