...
const handleSubmit = () => {
  ...
  createOrUpdateNetwork(networkDataClone.value.id as string, {
    ...
    routes: networkDataClone.value.routes,
  })
    .then(() => {
...
const addRoute = () => {
  if (networkDataClone.value) {
    if (!networkDataClone.value.routes) {
      networkDataClone.value.routes = []
    }
    networkDataClone.value.routes.push({
      target: '',
      via: '',
    })
  }
...
const removeRoute = (index: number) => {
  if (networkDataClone.value && networkDataClone.value.routes) {
    networkDataClone.value.routes.splice(index, 1)
  }
}
</script>
...
<template>
  <Dialog :open="open" @update:open="handleOpenChange">
    <DialogContent class="max-w-[95vw] sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle>{{ t('network.routes') }}</DialogTitle>
        <DialogDescription>
          {{ t('network.default') }} / {{ t('network.via.default') }}
        </DialogDescription>
      </DialogHeader>
      <div class="py-4" v-if="networkDataClone">
        <div class="space-y-3">
          <div v-for="(route, index) in networkDataClone.routes" :key="index" class="flex items-center gap-2">
            <Input type="text" v-model="route.target" :placeholder="t('network.default')"
              class="flex-1 border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0"
              autocomplete="off" />
            <Input type="text" v-model="route.via" :placeholder="t('network.via.notice')"
              class="flex-1 border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0"
              autocomplete="off" />
            <Button type="button" variant="destructive" size="icon" class="self-end" @click="removeRoute(index)">
              <img :src="deleteIcon" alt="delete" class="w-4 h-4 filter brightness-0 invert" />
            </Button>
          </div>
          <div v-if="!networkDataClone.routes || networkDataClone.routes.length === 0"
      ...
      <DialogFooter class="gap-2">
        ...
        <Button type="button" variant="secondary" @click="addRoute">
          {{ t('common.add') }}
        </Button>
